...
def _make_node_table_of_right_resolving_graph(graph):
    ...
    node_table = []
    for i in range(node_table_size):
        node_table.append([0] * (node_table_size - i))
        for j in range(node_table_size - i):
...
def gen_right_resolving_graph(graph):
    ...
    for i in range(right_resolving_node_size):
        subset_node = {}
        for j in label:
...
def gen_minimal_graph(graph, node_map=None):
    ...
    for I in range(minimal_node_size):
        for J in range(minimal_node_size):
            label = []
            for i in range(node_size):
                if node_map[i] == I:
                    for j in range(node_size):
...
def _add_edge_in_labeled_graph(initial_node, terminal_node, label, graph,
    ...
    if label not in edge_count[initial_node][terminal_node]:
...
def remove_edge_from_labeled_graph(graph, edge_count, threshold):
    ...
    for i in range(node_size):
        sum = 0
        for j in range(node_size):
            for k, n in edge_count[i][j].items():
                sum = sum + n
        for j in range(node_size):
...
def convert_trans_count(graph, conv_graph, node_map, edge_count, node_count):
    ...
    conv_edge_count = []
    for i in range(conv_node_size):
        conv_edge_count.append([])
        for j in range(conv_node_size):
...
def main():
    ...
    if s_list != []:
        ...
        edge_count, node_count = convert_trans_count(graph, minimal_graph,node_map, edge_count, node_count)
